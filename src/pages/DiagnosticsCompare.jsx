...
const DiagnosticsCompare = () => {
  ...
  if (items.length === 0) {
    return <div style={{ padding: '2rem', textAlign: 'center' }}>No items to compare. <button onClick={() => navigate('/diagnostics')}>Go Back</button></div>;
  }

  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f3f4f6', padding: '2rem' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', backgroundColor: 'white', borderRadius: '0.5rem', padding: '1rem' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              ...
              <tr>
                ...
                {items.map(item => <td key={item._id} style={{ padding: '0.75rem', border: '1px solid #e5e7eb' }}>₹{Math.round((item.min_price || item.discounted_price || item.price) * 0.9)}</td>)}
              </tr>
              <tr>
                <td style={{ padding: '0.75rem', border: '1px solid #e5e7eb', fontWeight: 'bold' }}>Report Time</td>
                {items.map(item => <td key={item._id} style={{ padding: '0.75rem', border: '1px solid #e5e7eb' }}>{item.turnaround_time_default_hours || item.report_time_hours || 24} hours</td>)}
              </tr>
              <tr>
