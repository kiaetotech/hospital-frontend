    ...
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px' }}>
              ...
              {expandedMainCat[category.code] && (
                <div style={{ backgroundColor: '#f9fafb' }}>
                  {category.subcategories.map(sub => (
                    <div key={sub.name} style={{ borderBottom: '1px solid #e5e7eb' }}>
                      {/* Subcategory Header */}
                      <div
                      ...
                      {expandedSubCat[`${category.code}_${sub.name}`] && (
                        <div style={{ padding: '10px 15px', display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
                          {sub.tests.map(test => (
                            <label key={test} style={{ display: 'flex', alignItems: 'center', gap: '8px', width: '220px', padding: '5px', cursor: 'pointer' }}>
                              <input
                                type="checkbox"
      ...
      {activeTab === 'packages' && (
        <div>
          ...
          <p>Full Body Checkup, Cardiac Care, Diabetes Profile - Coming Soon</p>
        </div>
      )}
