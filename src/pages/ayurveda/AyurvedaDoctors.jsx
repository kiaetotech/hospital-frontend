...
const AyurvedaDoctors = () => {
  ...
  const [filters, setFilters] = useState({
    ...
    minRating: '',
    maxFee: '',
    sortBy: 'rating' // rating | distance | fee-low | fee-high | experience
  });
  ...
  const allDoctors = [
    {
      ...
      about: '15+ years experience in authentic Panchakarma therapies. Specialized in chronic disease management.',
      available: true,
    ...
    },
    {
      ...
      about: 'Holistic Ayurvedic physician focusing on lifestyle disorders and women health.',
      available: true,
    ...
    },
    {
      ...
      about: 'Renowned Kerala Ayurveda practitioner with expertise in authentic Panchakarma.',
      available: false,
    ...
    },
    {
      ...
      city: 'Hyderabad',
      area: 'Banjara Hills',
      address: 'Reddy Skin & Ayurveda Clinic',
    ...
    },
    {
      ...
      consultationTypes: { online: true, clinic: true },
      education: 'BAMS, MD (Panchakarma) - Gujarat Ayurved University',
      about: 'Young and dynamic Panchakarma specialist focusing on lifestyle diseases.',
      available: true,
    ...
    },
    {
      ...
      city: 'Delhi',
      area: 'Hauz Khas',
      address: 'Singh Rasayana Clinic, Hauz Khas',
      coordinates: { lat: 28.5494, lng: 77.2001 },
      consultationTypes: { online: true, clinic: true },
      education: 'BAMS, MD (Rasayana) - BHU Varanasi',
      about: 'Specialist in rejuvenation therapy and anti-aging treatments.',
      available: true,
  ...
  // Get all unique cities and specializations for filters
  const cities = [...new Set(allDoctors.map(d => d.city))].sort();
  const specializations = [...new Set(allDoctors.map(d => d.specialization))].sort();
  const wellnessCenters = [...new Set(allDoctors.map(d => d.wellnessCenter))].sort();
    ...
    <div style={{ maxWidth: '1300px', margin: '0 auto', padding: '1rem' }}>
      ...
      <div style={{
        ...
        <h1 style={{ fontSize: '1.8rem', fontWeight: 'bold', marginBottom: '0.5rem' }}>
          👨‍⚕️ Find Ayurvedic Doctors
        </h1>
        <p style={{ opacity: 0.9, marginBottom: '1rem' }}>
          {filteredDoctors.length} doctors available • Filter by city, rating, distance & specialization
        </p>
      ...
      <div style={{
        ...
        <div style={{ 
          ...
          {/* City Filter */}
          <div>
            <label style={{ fontSize: '0.8rem', color: '#64748b', fontWeight: 'bold', display: 'block', marginBottom: '0.3rem' }}>
              📍 City
            </label>
            <select
              value={filters.city}
      ...
      <div style={{ 
        ...
        <p style={{ color: '#64748b', fontWeight: 'bold' }}>
          Showing <span style={{ color: '#4CAF50' }}>{filteredDoctors.length}</span> doctors
              ...
              <div style={{ flex: 1, minWidth: '250px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', flexWrap: 'wrap', gap: '0.5rem' }}>
                  <div>
                    ...
                    <p style={{ color: '#4CAF50', fontWeight: 'bold', fontSize: '0.9rem', marginBottom: '0.3rem' }}>
                      {doctor.specialization}
                    </p>
                    <p style={{ color: '#64748b', fontSize: '0.8rem', marginBottom: '0.3rem' }}>
                      🏥 {doctor.wellnessCenter}
                    </p>
                  </div>

                  {/* Distance & Availability Badge */}
                  <div style={{ textAlign: 'right' }}>
                    ...
                    <div style={{
                      ...
                      {doctor.available ? '🟢 Available' : '🟡 Next: ' + doctor.nextAvailable}
                    </div>
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.5rem' }}>
                  {doctor.consultationTypes.online && (
                    <span style={{ padding: '2px 8px', backgroundColor: '#e3f2fd', borderRadius: '10px', fontSize: '0.75rem', color: '#1565C0' }}>
                      💻 Online
                    </span>
                  )}
                  {doctor.consultationTypes.clinic && (
                    <span style={{ padding: '2px 8px', backgroundColor: '#e8f5e9', borderRadius: '10px', fontSize: '0.75rem', color: '#2E7D32' }}>
                      🏥 Clinic
                    </span>
                  )}
                </div>
