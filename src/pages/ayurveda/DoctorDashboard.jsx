...
const fetchComplaints = async () => {
  ...
  try {
    const token = localStorage.getItem('doctorToken');
    const response = await api.get('/ayurveda/bookings/doctor/complaints', {
    ...
    console.error('Fetch doctor complaints error:', error);
  } finally {
...
const fetchReviews = async () => {
  ...
  try {
    const token = localStorage.getItem('doctorToken');
    const response = await api.get('/ayurveda/bookings/doctor/reviews', {
      headers: { Authorization: `Bearer ${token}` }
    });
    if (response.data.success) {
...
const handleRespondToComplaint = async (bookingId, complaintId) => {
  ...
  try {
    ...
    const res = await api.put(
      `/ayurveda/bookings/${bookingId}/complaint/${complaintId}/doctor-respond`,
...
const handleResolveComplaint = async (bookingId, complaintId) => {
  ...
  try {
    const token = localStorage.getItem('doctorToken');
    const res = await api.put(
      `/ayurveda/bookings/${bookingId}/complaint/${complaintId}/doctor-resolve`,
      {},
      { headers: { Authorization: `Bearer ${token}` } }
    );
    if (res.data.success) {
...
const handleRespondToReview = async (bookingId) => {
  ...
  try {
    const token = localStorage.getItem('doctorToken');
    const res = await api.put(
      `/ayurveda/bookings/${bookingId}/review/doctor-respond`,
      { response: responseText },
      { headers: { Authorization: `Bearer ${token}` } }
    );
    if (res.data.success) {
          ...
          <div key={c.complaintId} className={`border rounded-lg p-4 ${
            ...
            <div className="flex justify-between items-start flex-wrap gap-2 mb-2">
              <div>
                ...
                <p className="text-sm text-gray-600 capitalize">Type: {c.bookingType?.replace(/_/g, ' ')}</p>
                {c.packageName && <p className="text-sm text-gray-600">Package: {c.packageName}</p>}
              </div>
              <div className="text-right">
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${
