                  ...
                  <div key={index} style={{ padding: '0.5rem 0', borderBottom: '1px solid #e5e7eb' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      ...
                      <span style={{ 
                        ...
                        borderRadius: '10px', 
                        fontSize: '0.7rem',
                        backgroundColor: booking.status === 'en_route' ? '#dbeafe' : booking.status === 'completed' ? '#dcfce7' : '#fef3c7',
                        color: booking.status === 'en_route' ? '#1e40af' : booking.status === 'completed' ? '#166534' : '#92400e'
                      }}>
                        {booking.status}
                      </span>
                    </div>
        ...
        return (
          <div>
            ...
            <ProviderTable
              columns={[
                ...
                { key: 'status', label: 'Status', render: (status) => (
                  <span style={{ 
                    ...
                    borderRadius: '10px', 
                    fontSize: '0.7rem',
                    backgroundColor: status === 'available' ? '#dcfce7' : status === 'on_duty' ? '#dbeafe' : '#fef3c7',
                    color: status === 'available' ? '#166534' : status === 'on_duty' ? '#1e40af' : '#92400e'
                  }}>
                    {status}
                  </span>
                )}
              ...
              data={vehicles}
              loading={loading}
              onEdit={(row) => alert(`Edit: ${row.vehicleNumber}`)}
              onDelete={(row) => { if (window.confirm(`Delete ${row.vehicleNumber}?`)) alert('Deleted'); }}
            />
          </div>
        );
        ...
        return (
          <div>
            ...
            <ProviderTable
              ...
              loading={loading}
              onView={(row) => alert(`View: ${row.bookingId}`)}
            />
          </div>
        );
