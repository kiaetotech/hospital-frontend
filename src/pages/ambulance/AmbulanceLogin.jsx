  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (activeTab === 'email') {
        ...
        res = await login({ email: form.email, password: form.password, role: 'ambulance' });
      } else {
      ...
      <div style={cardStyle}>
        ...
        <div style={tabContainerStyle}>
          <button onClick={() => { setActiveTab('email'); setError(''); }}
            style={{ ...tabStyle, background: activeTab === 'email' ? '#e53935' : 'transparent', color: activeTab === 'email' ? '#fff' : '#666' }}>
            ✉️ Email
          </button>
          <button onClick={() => { setActiveTab('phone'); setError(''); }}
            style={{ ...tabStyle, background: activeTab === 'phone' ? '#e53935' : 'transparent', color: activeTab === 'phone' ? '#fff' : '#666' }}>
            📱 Mobile OTP
          </button>
        </div>
              ...
              <div style={{ marginBottom: '10px' }}>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <button type="button" onClick={() => setShowPassword(!showPassword)}
                    style={passwordToggleStyle}>{showPassword ? '🙈' : '👁️'}</button>
                </div>
              </div>
            ...
            <>
              ...
              <div style={{ display: 'flex', gap: '8px', marginBottom: '10px' }}>
                <button type="button" onClick={handleSendOTP} disabled={otpCountdown > 0}
                  style={{
                    padding: '12px 16px', background: otpCountdown > 0 ? '#ccc' : '#2196f3',
                    color: '#fff', border: 'none', borderRadius: '10px', fontSize: '12px',
                    fontWeight: 600, cursor: otpCountdown > 0 ? 'not-allowed' : 'pointer', whiteSpace: 'nowrap'
                ...
                {otpSent && (
                  <input type="text" placeholder="6-digit OTP" value={form.otp}
                    ...
                    maxLength={6} style={{ ...inputStyle, flex: 1, letterSpacing: '6px', textAlign: 'center', fontSize: '18px', marginBottom: 0 }} />
                )}
              </div>
            </>
          )}
