...
const CorporateHRLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (activeTab === 'email') {
        ...
        res = await api.post('/auth/login', { email: form.email, password: form.password, role: 'corporate_hr' });
      } else {
        ...
        if (res.data?.success) res = await api.post('/auth/login', { phone: `+91${form.phone}`, role: 'corporate_hr', otpLogin: true });
      }
      if (res.data?.success) {
        localStorage.setItem('providerToken', res.data.token);
        localStorage.setItem('providerId', res.data.user?._id || '');
        localStorage.setItem('providerType', 'corporate_hr');
  ...
  const c = '#4527A0';
  return (
    <div style={{ minHeight:'100vh',background:'linear-gradient(135deg,#4527A0,#311B92)',display:'flex',alignItems:'center',justifyContent:'center',padding:'20px',fontFamily:'Arial' }}>
      <div style={{ width:'100%',maxWidth:'420px',background:'#fff',borderRadius:'20px',padding:'30px 24px',boxShadow:'0 20px 60px rgba(0,0,0,0.3)' }}>
        <div style={{ textAlign:'center',marginBottom:'24px' }}><span style={{ fontSize:'44px',display:'block' }}>🏢</span><h2 style={{ margin:'8px 0 0',fontSize:'22px',fontWeight:800 }}>Corporate HR Login</h2><p style={{ fontSize:'13px',color:'#888' }}>Employee Health Management</p></div>
        ...
        <div style={{ textAlign:'center',marginTop:'18px',paddingTop:'16px',borderTop:'1px solid #eee' }}><p style={{ fontSize:'13px',color:'#888',margin:0 }}>Don't have an account? <Link to="/corporate/enroll" style={{ color:c,fontWeight:700,textDecoration:'none' }}>Enroll Your Company</Link></p></div>
      </div>
    </div>
  );
};
